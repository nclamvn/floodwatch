#!/usr/bin/env python3
"""
VietnamNet RSS Scraper

Fetches disaster/weather news from VietnamNet RSS feeds and stores as Reports.
Source: VietnamNet (trust score 0.9)
"""
...
# VietnamNet RSS feeds for disaster/weather news
RSS_FEEDS = [
    "https://vietnamnet.vn/rss/thoi-su.rss",  # Current affairs - includes disasters
    "https://vietnamnet.vn/rss/xa-hoi.rss",  # Society
]
...
def scrape_vietnamnet_rss(dry_run: bool = False) -> int:
    """
    Scrape VietnamNet RSS feeds and store as Reports.
    ...
    """
    print(f"[{datetime.now()}] Starting VietnamNet RSS scraper...")
            ...
            for entry in feed.entries:
                try:
                    ...
                    dedup_fields = NewsDedupService.prepare_report_dedup_fields(
                        title, description, link or "vietnamnet.vn"
                    )

                    # Create report
                    report = Report(
                        type=report_type,
                        source=link or "vietnamnet.vn",
                        ...
                        trust_score=0.9,  # VietnamNet base trust score
                        status="new",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Scrape VietnamNet RSS feeds for disaster news")
    parser.add_argument("--dry-run", action="store_true", help="Show what would be inserted without saving")
    args = parser.parse_args()

    count = scrape_vietnamnet_rss(dry_run=args.dry_run)
