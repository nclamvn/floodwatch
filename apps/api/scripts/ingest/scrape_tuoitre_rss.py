#!/usr/bin/env python3
"""
Tuổi Trẻ RSS Scraper

Fetches disaster/weather news from Tuổi Trẻ RSS feeds and stores as Reports.
Source: Tuổi Trẻ (trust score 0.90)
"""
...
# Tuổi Trẻ RSS feeds for disaster/weather news
RSS_FEEDS = [
    "https://tuoitre.vn/rss/thoi-su.rss",  # News - includes disasters
    "https://tuoitre.vn/rss/xa-hoi.rss",    # Society - includes weather events
]

# Keywords for report type classification (same as VnExpress)
TYPE_KEYWORDS = {
    ReportType.ALERT: [
...
def scrape_tuoitre_rss(dry_run: bool = False) -> int:
    """
    Scrape Tuổi Trẻ RSS feeds and store as Reports.
    ...
    """
    print(f"[{datetime.now()}] Starting Tuổi Trẻ RSS scraper...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Scrape Tuổi Trẻ RSS feeds for disaster news")
    parser.add_argument("--dry-run", action="store_true", help="Show what would be inserted without saving")
    args = parser.parse_args()

    count = scrape_tuoitre_rss(dry_run=args.dry_run)
