#!/usr/bin/env python3
"""
Zing News RSS Scraper

Fetches disaster/weather news from Zing News RSS feeds and stores as Reports.
Source: Zing News (trust score 0.85)
"""
...
# Zing News RSS feeds for disaster/weather news
RSS_FEEDS = [
    "https://zingnews.vn/rss/xa-hoi.rss",  # Society - includes disasters
    "https://zingnews.vn/rss/thoi-su.rss",  # Current affairs
]
        ...
        if len(images) < 3:
            ...
            if description:
                ...
                for url in img_urls:
                    ...
                    elif url.startswith('//') and any(ext in url.lower() for ext in ['.jpg', '.jpeg', '.png', '.webp', '.gif']):
                        images.append('https:' + url)

    except Exception as e:
        pass  # Silently skip image extraction errors

    # Return max 5 images, filter out duplicates
...
def is_disaster_related(title: str, description: str = "") -> bool:
    ...
    exclude_keywords = [
        ...
        "kinh tế", "kinh te", "chứng khoán", "chung khoan"
    ]
...
def deduplicate_check(db, title: str, created_at: datetime, window_hours: int = 24) -> bool:
    ...
    existing = db.query(Report).filter(
        ...
        Report.source.like('%zing%')
    ).all()

    for report in existing:
        existing_normalized = ' '.join(report.title.lower().split())

        # If titles are identical, consider it a duplicate
...
def scrape_zing_rss(dry_run: bool = False) -> int:
    """
    Scrape Zing News RSS feeds and store as Reports.
    ...
    """
    print(f"[{datetime.now()}] Starting Zing News RSS scraper...")
        ...
        for feed_url in RSS_FEEDS:
            ...
            for entry in feed.entries:
                try:
                    ...
                    # If no images from RSS, try extracting from article page using article_extractor
                    if len(media_urls) == 0 and link:
                        try:
                            article_result = extract_article_hybrid(link, language='vi')
                            if article_result['success'] and article_result.get('images'):
                    ...
                    dedup_fields = NewsDedupService.prepare_report_dedup_fields(
                        title, description, link or "zingnews.vn"
                    )

                    # Create report
                    report = Report(
                        type=report_type,
                        source=link or "zingnews.vn",
                        ...
                        trust_score=0.85,  # Zing News base trust score
                        status="new",
                    ...
                    if dry_run:
                        ...
                        print(f"    Media: {len(media_urls)} images")
                        print(f"    URL: {link}")
                    else:
                        db.add(report)
                        db.commit()
                        new_reports_count += 1
                        media_info = f"({len(media_urls)} images)" if media_urls else "(no images)"
                        print(f"  [NEW] {report_type.value}: {title[:60]}... {media_info}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Scrape Zing News RSS feeds for disaster news")
    parser.add_argument("--dry-run", action="store_true", help="Show what would be inserted without saving")
    args = parser.parse_args()

    count = scrape_zing_rss(dry_run=args.dry_run)
