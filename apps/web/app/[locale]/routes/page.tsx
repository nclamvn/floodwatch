...
export default function RoutesPage() {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem(STORAGE_KEYS.ROUTE_FILTERS)
      if (stored) {
        const parsed = JSON.parse(stored)
        setFilters({ ...DEFAULT_ROUTE_FILTERS, ...parsed })
      }
    } catch (error) {
      console.warn('Failed to load route filters:', error)
    }
    setIsHydrated(true)
  }, [])
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(STORAGE_KEYS.ROUTE_FILTERS, JSON.stringify(filters))
    } catch (error) {
      console.warn('Failed to save route filters:', error)
    }
  }, [filters, isHydrated])
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm text-neutral-600 dark:text-neutral-400">
            <span className="px-3 py-1.5 rounded-xl bg-neutral-100 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 font-medium">
              {t('routeCount', { count: sortedRoutes.length })}
            </span>
            {searchQuery && (
              <span className="text-neutral-500">
                ({t('searchFor', { query: searchQuery })})
              </span>
            )}
          </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-neutral-700 dark:text-neutral-300 mb-2">
              {searchQuery ? t('noRouteFound') : t('stableTraffic')}
            </h3>
            ...
            {(searchQuery || filters.status.length > 0 || filters.province !== 'all') && (
              <button
                onClick={() => {
                  setSearchQuery('')
            ...
            <div className="flex-1">
              ...
              <ul className="text-sm text-amber-800 dark:text-amber-200 space-y-1.5">
                <li className="flex items-start gap-2">
                  ...
                  <span>{t('warnings.unverified')}</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-amber-500 mt-0.5">⚠</span>
                  <span>{t('warnings.recent')}</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-red-500 mt-0.5 font-bold">!</span>
                  <span>{t('warnings.call')}</span>
                </li>
              </ul>
            </div>
          </div>
