...
import { X, AlertTriangle, Info, CheckCircle, XCircle, Wind } from 'lucide-react'
import ReactMarkdown from 'react-markdown'

// Type definitions matching backend response
interface StormSummaryData {
  province: string  // Will be "Bão số 15 (Koto)"
  summary_text: string
...
interface StormSummaryModalProps {
  data: StormSummaryData | null
  isOpen: boolean
  onClose: () => void
  onStormPathClick: () => void
  onReportClick: (reportId: string) => void
}

export default function StormSummaryModal({
  data,
  isOpen,
  onClose,
  onStormPathClick,
  onReportClick
}: StormSummaryModalProps) {
          ...
          <div className="ml-4 flex items-center gap-2">
            ...
            <button
              ...
              className="
                ...
                px-4 py-2
              "
              aria-label="Xem đường đi của bão"
            >
              <Wind className="w-5 h-5" />
              <span className="hidden sm:inline text-sm font-medium">Đường đi của bão</span>
            </button>
            ...
            <button
              onClick={onClose}
              className="
                w-11 h-11 sm:w-10 sm:h-10 p-2 rounded-full
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-300 dark:scrollbar-thumb-white/20 scrollbar-track-transparent">
          ...
          <div className="mt-8 pt-6 border-t border-neutral-200 dark:border-white/10 text-center text-sm text-neutral-500 dark:text-neutral-50/50">
            <p>
              ...
              <br />
              Vui lòng kiểm tra thông tin trên các kênh chính thức của Trung tâm Dự báo KTTV Trung ương.
            </p>
          </div>
        </div>
      </div>
