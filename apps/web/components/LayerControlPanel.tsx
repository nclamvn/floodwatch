...
export default function LayerControlPanel({ visibility, onChange }: LayerControlPanelProps) {
  const layers = [
    { key: 'reports' as keyof LayerVisibility, Icon: MapPin, label: 'Báo cáo cộng đồng', color: 'text-blue-600' },
    { key: 'heavyRain' as keyof LayerVisibility, Icon: CloudRain, label: 'Mưa lớn', color: 'text-blue-500' },
    ...
    { key: 'damRelease' as keyof LayerVisibility, Icon: Dam, label: 'Xả đập', color: 'text-purple-600' },
  ...
  return (
    <div className="absolute top-20 left-4 z-40 w-72 bg-white/80 dark:bg-neutral-900/80 backdrop-blur-md rounded-lg shadow-sm border border-white/30 dark:border-neutral-700/30 overflow-hidden">
      {/* Header */}
      ...
      <div className="p-3 space-y-2 max-h-[calc(100vh-250px)] overflow-y-auto custom-scrollbar">
        ...
        <button
          onClick={toggleAll}
          className="w-full px-3 py-2 bg-blue-100/80 dark:bg-blue-900/20 hover:bg-blue-200/80 dark:hover:bg-blue-900/30 rounded-md text-sm font-medium text-blue-800 dark:text-blue-400 transition-colors flex items-center justify-between"
        >
        ...
        {layers.map(layer => {
          ...
          return (
            <label
              key={layer.key}
              className="flex items-center gap-2 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-neutral-800/50 cursor-pointer transition-colors group"
            >
              <input
                type="checkbox"
              ...
              <span className={`text-sm font-medium flex-1 ${visibility[layer.key] ? 'text-gray-900 dark:text-neutral-100' : 'text-gray-500 dark:text-neutral-500'}`}>
                {layer.label}
              </span>
